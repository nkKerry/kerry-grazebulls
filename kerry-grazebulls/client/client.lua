...
Citizen.CreateThread(function()
    for bullmenu, v in pairs(Config.Bull) do
        exports['rsg-core']:createPrompt(v.location, v.coords, RSGCore.Shared.Keybinds['J'], '' .. v.name, {
            type = 'client',
            event = 'kerry-grazebulls:client:bulljob',
            args = {},
        })
...
RegisterNetEvent('kerry-grazebulls:client:bulljob', function()
 exports['rsg-menu']:openMenu({
        ...
        },
        {
            ...
            params = {
                event = 'kerry-grazebulls:client:bullspawn',
                args = {}
            }
        },
		 {
            ...
            icon = 'fas fa-solid fa-cow',
            params = {
                event = 'kerry-grazebulls:client:bullreturn',
                args = {}
            }
        },
        {
...
RegisterNetEvent('kerry-grazebulls:client:bullspawn')
AddEventHandler('kerry-grazebulls:client:bullspawn', function()
    if starting then 
                ...
                options = {
                    {
                        ...
                        action = function()
                            TriggerEvent('kerry-grazebulls:client:lead', bullId)
                        end,
                    },
                    {
                        type = 'client',
						...
						targeticon = 'fas fa-solid fa-cow',
                        action = function()
                            TriggerEvent('kerry-grazebulls:client:pas', bullId)
                        end,
                    },
					{
                        type = 'client',
						...
						targeticon = 'fas fa-solid fa-cow',
                        action = function()
                            TriggerEvent('kerry-grazebulls:client:startanim', bullId)
                        end,
                    }
                },
                distance = 3.5,
        ...
        if randomChance <= Config.RandomChance then
            TriggerEvent('kerry-grazebulls:client:hunters')
        end
    end
end)

RegisterNetEvent('kerry-grazebulls:client:lead')
AddEventHandler('kerry-grazebulls:client:lead', function(bull)
...
RegisterNetEvent('kerry-grazebulls:client:startanim')
AddEventHandler('kerry-grazebulls:client:startanim', function(animal)
...
RegisterNetEvent('kerry-grazebulls:client:pas')
AddEventHandler('kerry-grazebulls:client:pas', function(animal)
	if IsPedDeadOrDying(animal, true) then
		RSGCore.Functions.Notify(Lang:t('error.dead_bulls'), 'error', 3000)
		return 
	end
...
RegisterNetEvent('kerry-grazebulls:client:hunters')
AddEventHandler('kerry-grazebulls:client:hunters', function()
...
RegisterNetEvent('kerry-grazebulls:client:bullreturn')
AddEventHandler('kerry-grazebulls:client:bullreturn', function()
    if isPasturing then
        ...
        if elapsedTime >= Config.WaitTimeBull then
            ...
            if isInZone then
				...
				RSGCore.Functions.Notify(Lang:t('success.success_bull'), 'success', 3000)
                    TriggerServerEvent('kerry-grazebulls:server:givemoney')
				else
